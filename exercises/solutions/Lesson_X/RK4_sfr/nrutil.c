...
/* allocate a MyFloat vector with subscript range v[nl..nh] */
{
	MyFloat *v;
...
/* allocate a MyFloat matrix with subscript range m[nrl..nrh][ncl..nch] */
{
	long i, nrow=nrh-nrl+1,ncol=nch-ncl+1;
	MyFloat **m;

	/* allocate pointers to rows */
	m=(MyFloat **) malloc((size_t)((nrow+NR_END)*sizeof(MyFloat*)));
	if (!m) nrerror("allocation failure 1 in matrix()");
	m += NR_END;
	m -= nrl;

	/* allocate rows and set pointers to them */
	m[nrl]=(MyFloat *) malloc((size_t)((nrow*ncol+NR_END)*sizeof(MyFloat)));
...
/* allocate a MyFloat matrix m[nrl..nrh][ncl..nch] that points to the matrix
...
/* allocate a MyFloat 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh] */
{
	long i,j,nrow=nrh-nrl+1,ncol=nch-ncl+1,ndep=ndh-ndl+1;
	MyFloat ***t;

	/* allocate pointers to pointers to rows */
	t=(MyFloat ***) malloc((size_t)((nrow+NR_END)*sizeof(MyFloat**)));
	if (!t) nrerror("allocation failure 1 in f3tensor()");
	t += NR_END;
	t -= nrl;

	/* allocate pointers to rows and set pointers to them */
	t[nrl]=(MyFloat **) malloc((size_t)((nrow*ncol+NR_END)*sizeof(MyFloat*)));
	if (!t[nrl]) nrerror("allocation failure 2 in f3tensor()");
	t[nrl] += NR_END;
	t[nrl] -= ncl;

	/* allocate rows and set pointers to them */
	t[nrl][ncl]=(MyFloat *) malloc((size_t)((nrow*ncol*ndep+NR_END)*sizeof(MyFloat)));
...
/* free a MyFloat vector allocated with vector() */
{
...
/* free a MyFloat vector allocated with dvector() */
{
	free((FREE_ARG) (v+nl-NR_END));
}

void free_matrix(MyFloat **m, long nrl, long nrh, long ncl, long nch)
/* free a MyFloat matrix allocated by matrix() */
{
...
/* free a MyFloat matrix allocated by dmatrix() */
{
	free((FREE_ARG) (m[nrl]+ncl-NR_END));
	free((FREE_ARG) (m+nrl-NR_END));
}
...
void free_f3tensor(MyFloat ***t, long nrl, long nrh, long ncl, long nch,
	long ndl, long ndh)
/* free a MyFloat f3tensor allocated by f3tensor() */
{
